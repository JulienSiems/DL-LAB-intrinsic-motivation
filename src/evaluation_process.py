...
def evaluate_folder(root_dir, file_name_list, folder_index, param_list, alpha, model_name_list, num_evals):
    ...
    args_for_filename = ['environment', 'map', 'extrinsic', 'intrinsic', 'fixed_encoder', 'ddqn', 'duelling', 'iqn',
                         'experience_replay', 'soft_update', 'n_step_reward', 'seed']
    ...
    Q_target_net = CNN(in_dim=state_dim, num_actions=num_actions, history_length=history_length,
                       duelling=duelling, iqn=iqn, activation=activation, embedding_dim=iqn_tau_embed_dim).to(
        device)
    ...
    tmp_inp = torch.zeros(
        size=(1, channels * (history_length if use_history_in_icm else 1), state_height, state_width))
    ...
    agent = DQNAgent(Q=Q_net, Q_target=Q_target_net, intrinsic_reward_generator=intrinsic_reward_network,
                     ...
                     prio_er_beta_decay=prio_er_beta_decay, state_dim=state_dim, iqn=iqn, iqn_n=iqn_n,
                     iqn_np=iqn_np,
